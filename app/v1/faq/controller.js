...
const { FAQ, Doctor, EstablishmentMaster, Hospital } = require("../../../models/index");
...
const allFAQ = async (req, res) => {
    try {
        ...
        if(id){
            ...
            data = await common.findAll(FAQ.model, { userId: new ObjectId(findDoctor.userId) });
        }
            ...
            data = await common.findAll(FAQ.model, { userId: new ObjectId(findEstablishmentUserId.userId) });
        }
        else{
            data = await common.findAll(FAQ.model,  { userType: constants.USER_TYPES.PATIENT });
        }
        return response.success(
